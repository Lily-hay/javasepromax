...
​    B. 形式参数：如果有多个参数，参数之间用分号隔开；如果没有参数，留空即可//用逗号隔开
...
B.
  ...
  method(d::funB);
C.
  method(Demo2::funB); //静态方法引用
D. 
  Demo2 d = new Demo2();
  method(d::funA);//实例方法引用
```
...
##### 9. 【多选题】能够完全匹配字符串"(010)-62661617"和"01062661617"的正则表达式包括（ACD） 
...
2. 什么是面向函数式编程？同面向对象编程有何区别？
   ...
   答：面向函数式编程（Functional Programming，FP）是一种**编程范式**，它强调使用**纯函数**、**不可变数据**和**表达式（而非语句）\**来构建程序\****

   

3. 请说明lambda表达式'()->{}'每个符号的含义？

   答：()里表示传递的参数，->表示链接的函数 ，{}里面是方法体

4. 什么是方法引用？什么情况下可以使用方法引用？

   答：Lambda表达式的一种简化写法，Lambda表达式内仅有一个实现方法，参数名与参数形式兼容，可以有静态方法引用、实例方法引用、构造器引用

5. 请说出冒泡排序和选择排序的排序原理？

   答：冒泡排序，依次将前后两值交换，排列整个序列的顺序

   选择排序：没次找到序列中的最小值(或最大值)

6. 正则表达式区分语言吗？有何作用？

   答：正则表达式核心思想一样，但不同语言的具体写法、扩展功能等有所不同
...
```java
// 下面代码是否能使用二分查找法，正确找到元素5的位置？如果不能，如何解决？//u不可以，二分查找要有序
public class Demo2 {
  public static void main(String[] args) {
    int[] arr = {5, 2, 3, 1, 4};
