...
1. 异常体系所涉及的各个类的单词：Exception、RuntimeException
2. 抛出异常单词：thorws
3. 制造异常单词：thorw
4. 集合体系各个接口单词：List、Set、Collection
5. 集合体系各个实现类单词： ArrayList、LinkedList、HashSet、LinkedSet、TreeSet
...
1. 什么是异常？为什么要有这么多异常类？
   ...
   答：异常就是编译和执行时可能出现的错误，为了让错误更明显

2. 异常的体系结构涉及到哪些类？并说明各个类？

   答：Exception，RunetimeException.

   ​		Exception编译时异常，RunetimeException运行时异常
...
4. 什么是迭代器？什么是并发修改异常？如何解决？
   ...
   ​		就是使用迭代器遍历时，又修改集合数据，就会出现并发修改异常

   ​		不在迭代器遍历时，使用修改集合的语句

5. 请简述ArrayList集合的底层数据结构，以及特点？

   答：数组，索引查询速度快，但是增删改查速度慢

6. 请简述LinkedList集合的底层数据结构，以及特点？

   答：双链表，查询速度慢，但是增删改查速度较快，对首尾进行增删改查速度很快
...
<img src="image/image-20220122133906756.png" alt="image"  />

答：输出结果为ArrayIndexOfBoundsExcrption,索引越界异常

因为arr数组的最大索引为2，而arrElement的索引为3
...
因为arr数组为null会报空指针异常，而且没有catch语句解决问题，会反馈到虚拟机中，结束虚拟机。finally会在中的语句总会执行，所以输出执行了finally语句了
